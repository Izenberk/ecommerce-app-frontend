...
export default function CartPage() {
    const { items, increment, decrement, remove, totalPrice } = useCartStore(
        useShallow((s) => ({
        items: s.items,
    ...
    return (
        ...
        {/* Empty-cart confirm dialog: 2 buttons, outside click closes */}
        <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
            <DialogContent>
            <DialogHeader>
                <DialogTitle>Your cart is empty</DialogTitle>
                <DialogDescription>
                Do you want to go shopping first, or proceed to checkout anyway?
                </DialogDescription>
            </DialogHeader>
            <DialogFooter>
                <Button
